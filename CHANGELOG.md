...
- **`PexesoooFlashcards.html`** — a fourth app in the suite: a flashcard trainer
that reads the **same set files** as the game and the printer, using the **same
file/folder loader and set preview**. A pexeso pair is read as one flashcard —
one side asks, the other answers. Fully client-side and offline-capable;
Czech / English UI. The set schema is unchanged (`version` stays `1`), so every
existing set works without modification.
  - **Deck mode** — one card at a time, enlarged and centred. Tap the card or
press **Space** to flip it and reveal the answer.
  - **Direction selector** — **A → B** or **B → A**. A set file records no side
semantics (the generator's Folder A / Folder B choice is the only ordering), so
the direction is chosen at read time. Each option renders the selected set's own
first pair, so the choice is made by looking rather than by remembering which
folder was A.
  - **Knew it / Repeat** — *Knew it* retires a card for good; *Repeat* drops it
into a leftover pile. When the queue empties, the leftover pile becomes the next
round. The session ends when every card has been retired, or early via
**End session**.
  - **Shuffle toggle** — on by default; off studies the set in its own order.
  - **Two-segment progress bar** — retired cards in `--ok`, cards waiting for a
repeat in `--bad`, with the round number beside the counter.
  - **Results** — total time, cards right first time, cards that needed a repeat
(with thumbnails of the prompt and answer), and cards not studied when a session
is ended early. From there: **Study again**, **Study the ones I missed** (restarts
with just that subset), or **Different set**.
  - **Keyboard** — Space or Enter flips; after the reveal, → or `1` is *Knew it*
and ← or `2` is *Repeat*. The card is focusable with a visible focus ring.
  - **Advisory** for sets built from 1:1 (identical) pairs, where every card would
answer itself — shown, not blocked, since such a set is still a legitimate pexeso.
  - **Launch parameters** — `lang=en|cs`, `dir=ab|ba`, `shuffle=on|off`.
...
- **All apps**
  - All apps display **v1.11.0**; the generator identifier in produced sets is
now `PexesoooGenerator/1.11.0`.
- **Docs** — `README.md` and `README.cs.md` now describe **four** apps: a
`PexesoooFlashcards.html` row in the file table, a **Study** / **Učení**
quick-start section, `dir` and `shuffle` rows in the launch-parameter table, and
updated app counts in the Set file format, Design, Security, Browser support,
Versioning and License sections.
